...
```
**ACTUALIZA el test existente de SP14 `'cancela UNA leg (en spot cancela toda la lista) y retorna'`** (HIGH-1
del plan-review): con 2 legs distintas ahora se cancelan ambas. Reemplázalo por:
```typescript
...
```
Los otros 3 tests de SP14 (OrderNotFound=éxito, NetworkError propaga, sin-ids=no-op) siguen válidos. NO
debilites: el contrato nuevo es "intenta todos los ids distintos".
...
```
**Imports del test (HIGH-2 del plan-review):** `setOrderExchangeId` e `insertBracketLeg` ya están en el
`vi.mock` de orders.ts del archivo, pero **no están importados** como bindings para las aserciones. Añade
al bloque de imports del test:
```typescript
import { setOrderExchangeId, insertBracketLeg } from '../../db/repositories/orders.ts';
```
El caso existente "sin OCO vivo → placeOco" que afirmaba `insertBracketLeg` con `getBracketLegs` vacío sigue
válido para el camino "sin legs → insert" — consérvalo.
...
```
**Actualiza el call site (MEDIUM-2 del plan-review):** en `reconcileOnePosition` (la función que llama a
`reprotectOrFlatten`), la línea final hoy es `return reprotectOrFlatten(deps, p);` — cámbiala a pasarle los
`legs` ya obtenidos en esa función (la variable `legs` filtrada por `exchangeOrderId` ya existe en
`reconcileOnePosition`):
```typescript
  return reprotectOrFlatten(deps, p, legs);
```
Añade `setOrderExchangeId` y el tipo `BracketLeg` a los imports de `exchange-reconcile.ts` (de
`../../db/repositories/orders.ts`) si no están.
...
export async function applyTrailingStop(deps: TrailingDeps, position: ReconcilePosition, newSl: number): Promise<void> {
  ...
  await lock(position.strategyId, position.symbol, deps.mode, async () => {
    ...
    if (!pos.decisionId) { await safeAudit('trailing_no_decision_id', { positionId: pos.id }); return; }  // MEDIUM-1: guarda interna (sin decisión no hay legs que recolocar)
...
```
Define `baseClient` cerca de los helpers del test (los métodos que el monitor usa vía mocks de order-state
están mockeados aparte, así que `baseClient` solo necesita ser un objeto base extensible):
```typescript
const baseClient = {} as Record<string, unknown>;
```
El `client` se pasa al monitor con `as never` (como en los tests existentes de SP13), así que el cast
absorbe la forma. Mantén los tests existentes de SP13 verdes (el trailing solo añade la rama con OCO vivo).
...
- [ ] **Step 4: Cablea el cast del cliente en el worker (HIGH-3 del plan-review — diff exacto)**

En `src/worker.ts`, añade dos imports (junto a los de las líneas 22-23):
```typescript
...
```
Y ensancha el cast del cliente en `realDeps()` (hoy línea 43: `const real = client as unknown as (RealClient & OrderStateClient);`):
```typescript
  const real = client as unknown as (RealClient & OrderStateClient & CancelOcoClient & PriceClient);
```
El runtime ya cumple (el cliente ccxt real tiene `cancelOrder`/`fetchTicker`). El tipo más ancho satisface
`MonitorRealDeps.client` (monitor) y sigue siendo asignable a `ReconcileDepsReal.client` (reconciler, que pide
el subconjunto). No se cambia ninguna otra llamada.
